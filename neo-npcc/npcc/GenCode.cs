    ...
    public class GenCode
    {
        ...
        public static bool GenerateCodeCustomMethods(NPCCompilerContext ctx, int classIndex, List<NPCClassInterfaceInfo> listClassInterfaces)
        {
            ...
            foreach (NPCClassInterfaceInfo cii in listClassInterfaces)
            {
                ...
                {
                    if (Trace.Warning) Console.WriteLine("**INFO*** Using custom methods template found in model class project.\t" + classModelFullyQualifiedFileName);
                ...
                {                    
                    ...
                    if (Trace.Warning) Console.WriteLine("**INFO*** Using embedded custom methods template.\t" + resname);
                    textTemplate = Helpers.GetTextResource(resname);
                }
                text = textTemplate.Replace(ctx.listModuleInfo[0].moduleModelClassProjectName, ctx.listModuleInfo[0].moduleTargetProjectName);
                text = text.Replace("#PROGRAMNAME#", Program.ProgramName);
                text = text.Replace("#PROGRAMVERSION#", Assembly.GetEntryAssembly().GetName().Version.ToString());
                text = text.Replace("#NOWDATETIME#", DateTime.Now.ToString());
                ...
                if (File.Exists(classModelFullyQualifiedFileNameI))
                {
                    ...
                    textTemplate = File.ReadAllText(classModelFullyQualifiedFileNameI);
